...
export default function BlogCard({
  ...
  return (
    <Link
      href={`/blog/${post.slug ?? ""}`}
      className="flex flex-col shadow-md shadow-indigo-100 hover:shadow-indigo-200 md:flex-row-reverse gap-6 border-b p-5 hover:bg-gray-50 dark:hover:bg-slate-900/30 rounded-md transition-colors relative">
      {/* ✅ Image di kanan */}
      {post.cover && (
        <div className="md:w-1/3 flex-shrink-0">
          <img
            src={post.cover}
            alt={post.title}
            className="w-full h-40 md:h-full object-cover rounded-md shadow-md shadow-indigo-200"
          />
        </div>
      )}
